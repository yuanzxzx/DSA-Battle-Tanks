...
class Cannon:
    ...
    """
    def __init__(self,position,type_gun: CannonType): # Initial position and cannon type
    ...
    def __str__(self): # Updates Bullet Loaded Counts
        return "BULLETS AVAILABLE: " + str(self.type_gun.count_available)

    def check_available_bullets(self): 
        """ check available. """
        if self.type_gun.count_available > 0 or self.type_gun.limit is True: #Fires only if it has ammo
            return True
        return False
