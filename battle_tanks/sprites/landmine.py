...
class LandMine(pg.sprite.Sprite):
    ...
    def __init__(self, x: int, y: int, owner_name: str):
        ...
        self.image = pg.Surface((self.SIZE, self.SIZE), pg.SRCALPHA) # Image of landmine
        self._draw(armed=False)
        self.rect = self.image.get_rect(topleft=(x, y))

    def _draw(self, armed: bool): # Draws the landmine
        ...
        # spikes
        for i in range(8): # Creates spikes for the landmine
    ...
    def update(self, players: list, owner_name: str): # Updates the landmine
        if not self.armed:
            self._arm_timer -= 1
    ...
    def check_trigger(self, players: list): # Checks if the landmine is triggered
        """Returns list of players hit if triggered, else empty list."""
        if not self.armed:
            return []
        mine_rect = pg.Rect(self.world_x, self.world_y, self.SIZE, self.SIZE)
        for player in players: # Loops through players to check if they are in the explosion radius
